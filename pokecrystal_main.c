...
#include "assets_manifest_pokecrystal.h"
extern uint8_t pokecrystal__rom_data[];
extern const size_t pokecrystal__rom_size;
static const GBGameAssets GB_POKECRYSTAL_GAME = {
    .game_id = "pokecrystal",
    .rom_filename = "roms/pokecrystal.gbc",
    .rom_data = pokecrystal__rom_data,
    .rom_size = 2097152u,
    .expected_sha1 = { 0xf2, 0xf5, 0x22, 0x30, 0xb5, 0x36, 0x21, 0x4e, 0xf7, 0xc9, 0x92, 0x4f, 0x48, 0x33, 0x92, 0x99, 0x3e, 0x22, 0x6c, 0xfb },
    .manifest = POKECRYSTAL_ASSETS_MANIFEST,
    .manifest_count = POKECRYSTAL_ASSETS_MANIFEST_COUNT,
};
...
int pokecrystal_main(int argc, char* argv[]) {
    ...
    GBConfig runtime_config = *pokecrystal_default_config();
    ...
    if (differential_mode) {
        ...
        gb_context_set_save_id(generated_ctx, "pokecrystal");
        gb_context_set_save_id(interpreted_ctx, "pokecrystal");
        pokecrystal_init(generated_ctx);
        pokecrystal_init(interpreted_ctx);
    ...
    gb_context_set_save_id(ctx, "pokecrystal");
    ...
    if (!gb_chdir_to_exe_dir()) {
        fprintf(stderr, "[pokecrystal] chdir to exe dir failed\n");
        return 1;
    }
    if (!gb_load_assets(&GB_POKECRYSTAL_GAME)) {
        fprintf(stderr, "[pokecrystal] failed to load ROM/assets\n");
        return 1;
    }
#ifdef GB_HAS_SDL2
    gb_platform_set_game_id(ctx, GB_POKECRYSTAL_GAME.game_id);
#endif
    pokecrystal_init(ctx);
    int exit_code = 0;
